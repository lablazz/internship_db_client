...
function CountdownTEA() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      await axios
        .post("https://server.stat-interndb.com/manageTime", {
          action: "fetch",
        })
        .then((res) => {
  ...
  const handleOnAdd = (event)=>{
    ...
    axios.post("https://server.stat-interndb.com/manageTime", {
  ...
  const handleOnDel = async (id) => {
    ...
    await axios.post("https://server.stat-interndb.com/manageTime", {
        action: 'del', id
    }).then((res)=>{
        const {status, msg} = res.data
        if (status == 'success') {
